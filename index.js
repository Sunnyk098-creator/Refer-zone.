...
bot.on('message', async (msg) => {
    ...
    // --- Dev Command ---
    if (text === '/dev') {
        return bot.sendMessage(chatId, "made by sunny");
    }
    ...
    if (userId === ADMIN_ID && userStates[userId]) {
        ...
        try {
            if (state === 'ADD_CHANNEL') {
                ...
                // Parse t.me links to standard @username format
    ...
    if (userId === ADMIN_ID) {
        ...
        if (text === '📢 Add Channel') {
            ...
            return bot.sendMessage(chatId, "Send the channel username (e.g., @mychannel) or Invite Link (https://t.me/...):");
        }
        if (text === '❌ Remove Channel') {
            ...
            return bot.sendMessage(chatId, "Select a channel to remove:", {
    ...
    if (userStates[userId] && userStates[userId].step === 'WITHDRAW_AMOUNT') {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                // Alert Admin regarding successful withdrawal
...
bot.on('callback_query', async (query) => {
    ...
    if (data.startsWith('remove_ch_')) {
        ...
        // Optionally edit the message to show it was removed
        bot.editMessageText("✅ Channel removed.", {
            chat_id: chatId,
