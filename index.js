    ...
    for (let ch of channels) {
        ...
        try {
            ...
            if (!['member', 'administrator', 'creator'].includes(member.status)) {
                // Check if they requested to join (pending)
            ...
            // Fallback: check pending request if chat member check fails
            const pendingReq = await get(ref(db, `join_requests/${chId}/${userId}`));
            if (!pendingReq.exists()) {
                allJoined = false;
                pending.push(ch);
    ...
    if (text.startsWith('/start')) {
        ...
        if (!userData) {
            userData = {
                ...
                verified: false,
                rewardGiven: false,
                wallet: "Not Linked" // Fix for Firebase dropping null keys
            };
...
bot.on('callback_query', async (query) => {
    ...
    if (data === 'check_verification') {
        ...
        // 🛠 FIX: Restore missing properties if Vercel App overwrote them
        ...
        if (userData.verified === true || userData.verified === "true") {
            ...
            if (userData.rewardGiven !== true && userData.referredBy && userData.referredBy !== false) {
                ...
                if (inviterSnap.exists()) {
                    ...
                    // Force Number parsing to prevent String bug
