...
bot.on('message', async (msg) => {
    ...
    if (isAdminUser) {
        ...
        // --- ADMIN LEADERBOARD CODE (Full ID with Clickable Link) ---
        if (text === '🏆 Leaderboard') {
            const usersSnap = await get(ref(db, 'users'));
    ...
    // --- NEW USER COMMANDS (Without Emojis) ---
    ...
    if (text === 'Refer earn') {
        ...
        const opts = {
            reply_markup: {
                inline_keyboard: [
                    [{ text: "🏆 View Top Leaderboard 🌟", callback_data: "show_user_leaderboard" }]
                ]
            }
        };
    ...
    if (text === 'Link wallet') {
        ...
        return bot.sendMessage(chatId, "📱 Please enter your 10-digit wallet number to receive withdrawals:");
    }
    ...
    if (userStates[userId] && userStates[userId].step === 'LINK_WALLET') {
        ...
        await update(ref(db, `users/${userId}`), { wallet: walletNumber });
        delete userStates[userId];
        return bot.sendMessage(chatId, `✅ Wallet number ${walletNumber} linked successfully! You can now withdraw easily.`, USER_MENU);
    }
    ...
    if (text === 'Withdraw') {
        ...
        if (!userData.wallet) {
            return bot.sendMessage(chatId, "❌ You haven't linked a wallet yet.\n\nPlease click on 'Link wallet' first to save your number.", USER_MENU);
        }
    ...
    if (userStates[userId] && userStates[userId].step === 'WITHDRAW_AMOUNT') {
        ...
        const walletNumber = userData.wallet; // Get the permanently saved wallet number
        delete userStates[userId];
...
bot.on('callback_query', async (query) => {
    ...
    if (data === 'show_user_leaderboard') {
        ...
        top10.forEach((user, index) => {
            ...
            // Masking logic: 123*****901
            if (idStr.length > 6) {
                maskedId = idStr.substring(0, 3) + "*****" + idStr.substring(idStr.length - 3);
            } else {
                maskedId = idStr.substring(0, 1) + "***" + idStr.substring(idStr.length - 1);
            }
