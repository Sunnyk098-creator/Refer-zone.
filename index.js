...
bot.on('message', async (msg) => {
    ...
    const text = msg.text || ''; // Ensure text is defined for forwarded messages
        ...
        if (isAdminUser) {
            ...
            try {
                if (state === 'ADD_CHANNEL') {
                    ...
                    // 🆕 FIX: Allow forwarding a message from a public channel to add it
    ...
    if (isAdminUser) {
        ...
        if (text === '📢 Add Channel') {
            ...
            // 🆕 Updated instructions to include message forwarding
        ...
        if (text === '🏆 Leaderboard') {
            ...
            const usersSnap = await get(ref(db, 'users')); // Live DB Call
            if (!usersSnap.exists()) {
...
bot.on('callback_query', async (query) => {
    ...
    if (data === 'show_user_leaderboard') {
        // 🆕 FIX: Added realtime updating feedback
        bot.answerCallbackQuery(query.id, { text: "🔄 Fetching live data..." }).catch(()=>{});
        
        const usersSnap = await get(ref(db, 'users')); // Live DB fetch
        ...
        try {
            ...
            // Ignore "Message is not modified" if live data is exactly the same as displayed data
        }
        return;
    }
