                ...
                if (state === 'ADD_CHANNEL') {
                    ...
                    if (val.includes(' ')) {
                        ...
                        if (!channelId.startsWith('-100')) {
                            return bot.sendMessage(chatId, "❌ Private Chat ID hamesha -100 se start hota hai.\n\n💡 *Tip:* Bot ko pehle apne channel me admin banayein, wo khud turant aapko ID message kar dega!", { parse_mode: 'Markdown' });
                        }
                    } else {
                        channelId = val;
                    ...
                    try {
                        ...
                        bot.sendMessage(chatId, `❌ Channel invalid hai ya bot usme add nahi hai.\n\n*Note:* Private link se direct ID nahi nikalti. Bas bot ko apne private channel me Admin banayein, aur bot aapko khud ID bhej dega!`, { parse_mode: 'Markdown' });
                    }
...
// ==========================================
// AUTO-APPROVE PRIVATE CHANNEL JOIN REQUESTS
// ==========================================
bot.on('chat_join_request', async (request) => {
    ...
    try {
        // Approve the user's join request instantly
        await bot.approveChatJoinRequest(chatId, userId);
        
        // Optionally, alert the user that they are approved
        bot.sendMessage(userId, `✅ Your request to join **${request.chat.title}** has been approved!\n\n👉 Now click on **'✅ Verify Join'** again.`, { parse_mode: 'Markdown' }).catch(() => {});
        
    } catch (error) {
        console.error("Error auto-approving join request:", error.message);
    }
});
...
bot.on('my_chat_member', async (msg) => {
    if (msg.chat.type === 'channel') {
        ...
        // Jab bot ko kisi bhi channel me admin banaya jaye
        if (newStatus === 'administrator' || newStatus === 'creator') {
            ...
            // Sabhi admins ki ek unique list banayenge (Set use karke taaki duplicate messages na jaye)
            const adminIds = new Set();
            adminIds.add(ADMIN_ID); // Master admin ko fix add kiya
            
            try {
                // Firebase Database se baaki banaye hue admins nikalenge
                const snap = await get(ref(db, 'admins'));
                if (snap.exists()) {
                    const otherAdmins = snap.val();
                    for (let id in otherAdmins) {
                        if (otherAdmins[id] === true) {
                            adminIds.add(Number(id)); // Sub-admins ko add kiya
                        }
                    }
                }
                
                // Ab list me jitne bhi admins hain, sabko loop chalake message bhejenge
                ...
                // Fallback: Agar database fetch me koi issue aaye, toh kam se kam Master Admin ko zaroor jaye
