...
async function getSettings() {
    ...
    const defaultSettings = {
        ...
        minWithdraw: 10,
        maxWithdraw: 100,
        apiUrl: "https://site.com/api.php?paytm={wallet}&amount={amount}&comment=Payout", // Updated default
        referAmount: 5,
        minRefer: 1,
        bonusAmount: 1
    };
...
bot.on('message', async (msg) => {
    ...
    if (userStates[userId]) {
        ...
        if (state === 'WITHDRAW_AMOUNT') {
            ...
            const walletNumber = userData.wallet; 
            
            if (isNaN(amt) || amt < settings.minWithdraw || amt > settings.maxWithdraw || amt > userData.balance) {
                delete userStates[userId]; // Reset state on error so user isn't stuck
                return bot.sendMessage(chatId, "❌ Invalid amount or insufficient balance. Try again via 'Withdraw'.", USER_MENU);
            }

            await update(userRef, { balance: userData.balance - amt });
            bot.sendMessage(chatId, "⏳ Processing withdrawal...", USER_MENU);
            delete userStates[userId]; // Clear state

            try {
                // Modified to support {wallet} as well as {number} placeholder
                const finalUrl = settings.apiUrl.replace('{number}', walletNumber).replace('{wallet}', walletNumber).replace('{amount}', amt);
    ...
    if (text === 'Link wallet') {
        ...
        // Extracting base URL from the saved API Gateway
        let siteDomain = "";
        try {
            if (settings.apiUrl) {
                const urlObj = new URL(settings.apiUrl);
                siteDomain = urlObj.origin; // Extracts just https://site.com
            }
        } catch (e) {}

        const msg = `💳 Send Wallet Registered Number\n\n🔗 Link : ${siteDomain}`;
    ...
    if (text === 'Withdraw') {
        ...
        // ❌ Yaha check ho raha hai - Agar wallet nahi hai to error prompt
        if (!userData.wallet) {
        ...
        // ✅ Agar wallet hai, directly pucho kitna amount nikalna hai
    ...
    if (isAdminUser) {
        ...
        if (text === '⚙️ Set API Gateway') {
            ...
            // Updated API instruction text
            const msg = `Send the API URL.\n\n*Format Example:*\n\`https://site.com/api.php?paytm={wallet}&amount={amount}&comment=Payout\`\n\n_(Note: \`&comment=Payout\` is optional. Ensure you use \`{wallet}\` and \`{amount}\` tags.)_`;
