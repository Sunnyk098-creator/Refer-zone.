...
const BOT_NAME = "Black tree🌴🤑"; // 🆕 Updated Name
...
async function checkChannels(userId) {
    ...
    let allJoined = true;
    let pending = []; // 🆕 Only unjoined channels will be stored here
    ...
    for (let ch of channels) {
        ...
        try {
            ...
            // Error handling ignored for privacy
        }
        ...
        if (!isJoinedOrRequested) {
            allJoined = false;
            pending.push(ch); // 🆕 Only channels NOT joined get pushed
        }
    }
...
bot.on('callback_query', async (query) => {
    ...
    // 🆕 FIX: Do not answer query globally if it's verify_join so we can show proper alerts
    if (data !== 'verify_join') {
    ...
    if (data === 'verify_join') {
        ...
        if (!channelStatus.allJoined) {
            ...
            // Refresh keyboard to REMOVE channels they already joined
            try {
                ...
                // Ignore "Message is not modified" error if they didn't join anything new
            }
        } else {
            // Send verified alert and prompt verification
...
// ==========================================
// STOP AUTO-APPROVE (CHECK PENDING JOIN REQUESTS)
// ==========================================
