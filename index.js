...
async function checkChannels(userId) {
    ...
    for (let ch of channels) {
        ...
        try {
            ...
            explicitStatus = member.status;
            if (['member', 'administrator', 'creator'].includes(member.status)) {
                isJoinedOrRequested = true;
            }
        } catch (e) {
            // User is not in chat, or chat is private and user hasn't joined
        }

        // Only check pending requests if the user hasn't explicitly "left" or been "kicked"
        if (!isJoinedOrRequested && explicitStatus !== 'left' && explicitStatus !== 'kicked') {
            const pendingReq = await get(ref(db, `join_requests/${chId}/${userId}`));
...
bot.on('callback_query', async (query) => {
    ...
    // 🛠 UPDATED: Dynamic Button Removal when clicking 'verify_join'
    if (data === 'verify_join') {
        const channelStatus = await checkChannels(userId);
        if (!channelStatus.allJoined) {
            bot.answerCallbackQuery(query.id, { text: "❌ Please join the remaining channels!", show_alert: true });
            // Update the inline keyboard to only show the channels they still haven't joined/requested
            bot.editMessageReplyMarkup(generateJoinKeyboard(channelStatus.channels).reply_markup, {
                chat_id: chatId,
        ...
        if (userData.verified === true || userData.verified === "true") {
            ...
            if (userData.rewardGiven !== true && userData.referredBy && userData.referredBy !== false) {
                ...
                if (inviterSnap.exists()) {
                    ...
                    let oldBal = Number(inviterData.balance) || 0;
                    let oldRef = Number(inviterData.referrals) || 0;
                    let refAmt = Number(settings.referAmount) || 0;

                    await update(inviterRef, {
                        balance: oldBal + refAmt,
                        referrals: oldRef + 1
                    });
                    
                    bot.sendMessage(inviterId, `🎉 ₹${refAmt} Credited to Your Balance! Invited : ${userId}`).catch(()=>{});
                }
            }
        } else {
...
// ==========================================
// 🛠 UPDATED: STOP AUTO-APPROVE (CHECK PENDING JOIN REQUESTS)
// ==========================================
bot.on('chat_join_request', async (request) => {
    ...
    try {
        ...
        // 🛠 Notify User so they know to return to the bot and click Verify
        bot.sendMessage(userId, "✅ *Your join request was recorded!*\n\nPlease click the '✅ Verify Join' button in the bot menu to continue.", { parse_mode: 'Markdown' }).catch(()=>{});
