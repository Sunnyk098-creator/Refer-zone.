...
bot.on('message', async (msg) => {
    ...
    if (text === '💰 Balance') {
        return bot.sendMessage(chatId, `💰 *Your Balance:* ${userData.balance}\n👥 *Total Referrals:* ${userData.referrals}`, { parse_mode: 'Markdown' });
    }

    if (text === '👥 Refer') {
        const refLink = `https://t.me/${(await bot.getMe()).username}?start=${userId}`;
        return bot.sendMessage(chatId, `🔗 *Your Referral Link:*\n\`${refLink}\`\n\n_Tap to copy_\n\n🎁 You get ${settings.referAmount} once they verify!`, { parse_mode: 'Markdown' });
    }
    ...
    if (text === '🏧 Withdraw') {
        ...
        return bot.sendMessage(chatId, `💸 Enter amount to withdraw (Min: ${settings.minWithdraw}, Max: ${settings.maxWithdraw}):`);
    }
    ...
    if (data === 'check_verification') {
        ...
        if (userData.verified) {
            ...
            if (!userData.rewardGiven && userData.referredBy) {
                ...
                bot.sendMessage(userData.referredBy, `🎉 You got ${settings.referAmount} reward for a successful referral!`);
            }
        } else {
            bot.sendMessage(chatId, "❌ Verification not completed. Please open the Mini App and complete the process.");
        }
    }
});
